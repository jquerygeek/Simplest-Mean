 ...
 *
 * Note: This is both HTTP and HTTPS server. Please, READ THE COMMENTS carefully. If you want only HTTP server, use app.js file.
 * 
 */
...
/**
 *
 * In the following two lines, use httpapp, if your application does not use ssl certificates and you don't want HTTPS server.
 * You can use app, if your application uses ssl certificates and you want HTTPS server. I am using both because I want to
 * handle the case, if a person types http in URL of my web application, it is automatically taken to https.
 *
 */
 ...
 *
 * Note: Pass httpapp as argument here if you don't want https server.
 *
 */
...
/**
 *
 * Whenever a user types http in the URL, the following code redirects him/her to https.
 * This is just simple http application which is responding to all URLS (please notice that *).
 * This might be silly method to handle this situation, but this is all what I could do.
 *
 * NOTE: Replace the URL with your own or https://localhost:8443. 
 *
 * Please remove the following code if you don't want https server and simply want to run http server.
 *
 */

httpapp.get('*',function(req,res){  
    res.redirect('https://www.mywebapplication.com'+req.url)
});
...
/**
 *
 * If you have a secure server and you have purchased your ssl certificates. Just replace the file names in the following
 * code with your own file names. These files must be stored in directory called security. For now, the security folder
 * is empty. 
 *
 * Note: Remove this code, if you don't want to use certificates and you don't want https server.
 *
 */

var options = {
  ca: fs.readFileSync('security/bundle-name-given-by-godaddy.crt'),
  key: fs.readFileSync('security/name-of-your-key.key'),
  cert: fs.readFileSync('security/name-of-your-certificate.crt')
};
...
/**
 *
 * Here I have created the two servers, one is http and other is https. Notice, the options object created above is being used
 * in the https server. 
 *
 * Note: If you don't want to use https, remove the line containing var httpsServer.
 *
 */
 ...
 *
 * Note: Please, pass server to listen() function, if you don't want https server.
 *
 */
...
/**
 *
 * Both http and https servers start to listen here. 
 *
 * Note: If you don't want https server just remove the httpsServer.listen() code below. Also remove
 * many things above in order to make it http server and not https server. READ THE ABOVE COMMENTS.
 *
 * If you need HTTPS server, don't change this code.
 *
 */
